...
void print_beacon(beaconinfo beacon)
{
  if (beacon.err != 0) {
    ...
    Serial.printf("   %4d\r\n", beacon.rssi);
    */
    Serial.printf("beacon,%s,%02x%02x%02x%02x%02x%02x,%2d,%4d\n",beacon.ssid,beacon.bssid[0],beacon.bssid[1],beacon.bssid[2],beacon.bssid[3],beacon.bssid[4],beacon.bssid[5],beacon.channel,beacon.rssi);
  }
}
  ...
  if (ci.err != 0) {
    ...
    for (u = 0; u < aps_known_count; u++) {
      if (!memcmp(aps_known[u].bssid,ci.bssid,ETH_MAC_LEN)) {
        Serial.printf("client,%02x%02x%02x%02x%02x%02x,%s,%02x%02x%02x%02x%02x%02x,%3d,%4d\n",
        ci.station[0],ci.station[1],ci.station[2],ci.station[3],ci.station[4],ci.station[5],
        aps_known[u].ssid,
        ci.ap[0],ci.ap[1],ci.ap[2],ci.ap[3],ci.ap[4],ci.ap[5],
        aps_known[u].channel,ci.rssi);
        break;
      }
    }
